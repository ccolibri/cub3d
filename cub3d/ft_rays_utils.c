...
static t_ray_utils vertical(t_all *all, t_ray ray)
{
    ...
    while (v.x >= 0 && v.x <= all->frame.w && v.y >= 0 && v.y <= all->frame.h)
    {
        if (is_wall(v.x - (ray->is_left ? 1 : 0), v.y, all) == TRUE)
        {
...
static t_ray_utils horisontal(t_all *all, t_ray ray)
{
    ...
    while (h.x >= 0 && h.x <= all->frame.w && h.y >= 0 && h.y <= all->frame.h)
    {
        if (is_wall(h.x, h.y - (ray->is_up ? 1 : 0), all) == TRUE)
        {
