...
static t_render_utils create_stripe(t_all *all, int i, t_img tex)
{
    t_render_utils col;
    ...
    col.tex_pos = (col.top - all->frame.h / 2.0 + col.height / 2.0) * col.step;
    col.y = (int)col.top;
    return (col);
}
    ...
    while (i < all->frame.w)
    {
        ...
        while (c.y < c.top + c.height)
        {
            ...
            if (c.y >= 0 && c.y <= all->frame.h && i >= 0 && 1 <= all->frame.w)
                mlx_pixel_put(&all->img, i, c.y, *(unsigned int *)c.color);
            c.y++;
        }
        i++;
    }
